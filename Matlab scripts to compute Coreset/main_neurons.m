...
A = dlmread('/home/khalid/Datasets/neurons_processed.txt', ',');
size(A)
A = sparse(A);
n = size(A, 1);
%A = A(1:floor(n/2),:);
...
%
    ...
    fileName = strcat('/home/khalid/Datasets/neurons_beforePCa_coreset_', num2str(m), '.txt')
    indexName = strcat('/home/khalid/Datasets/neurons_beforePCA_coreset_idx_', num2str(m), '.txt')
    dlmwrite(fileName, X, 'delimiter', ',');
    %dlmwrite(indexName, Idx);
